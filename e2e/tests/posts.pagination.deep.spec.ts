...
const seedBody = (i: number) =>
  `Deep pagination post ${i.toString().padStart(2, '0')}`
...
test('pagination: Load more flips to "Loading…" mid-fetch and is removed after the final page', async ({
  ...
  // paginated feed calls (those carrying a `cursor` query parameter) by
...
test('pagination: three-page walk renders all 41 seeded bodies', async ({
  page,
  apiClient,
}) => {
  const aliceInput = randomSignupInput({ displayName: 'Alice' })
  ...
  for (const text of renderedTexts) {
    ...
    expect(
      match,
      `rendered card text must include a seeded body: ${text}`,
    ).toBeDefined()
  ...
  for (const body of seededBodies) {
    expect(
      ...
      `seeded body must be rendered: ${body}`,
    ).toBe(true)
  }
})
