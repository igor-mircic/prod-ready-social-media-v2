...
- [ ] 1.1 Create `backend/src/main/resources/db/migration/V5__create_feed_entries.sql` declaring a `feed_entries` table with columns `recipient_id UUID NOT NULL`, `post_id UUID NOT NULL`, `author_id UUID NOT NULL`, `created_at TIMESTAMPTZ NOT NULL`.
- [ ] 1.2 Declare the composite primary key `PRIMARY KEY (recipient_id, post_id)` in the same `CREATE TABLE` statement.
- [ ] 1.3 Declare the `recipient_id` foreign key as `REFERENCES users(id) ON DELETE CASCADE`. Do NOT use `RESTRICT` or `SET NULL`.
- [ ] 1.4 Declare the `post_id` foreign key as `REFERENCES posts(id) ON DELETE CASCADE`.
- [ ] 1.5 Create the read-index `CREATE INDEX feed_entries_read_idx ON feed_entries (recipient_id, created_at DESC, post_id DESC);` — backs the keyset page query.
- [ ] 1.6 Create the scrub-index `CREATE INDEX feed_entries_author_idx ON feed_entries (recipient_id, author_id);` — backs the unfollow scrub.
- [ ] 1.7 Confirm by `psql` (or the IT bootstrap) that `flyway:migrate` against an empty database leaves `feed_entries` in the expected shape and both indexes are present. Sanity-check the FKs by hand: deleting a `users` row removes the user's `feed_entries` rows; deleting (hard) a `posts` row removes any `feed_entries` referencing it.

## 2. Backend: `feed/` package — entity, id, repository

- [ ] 2.1 Create package `backend/src/main/java/com/prodready/social/feed/`.
- [ ] 2.2 Create `feed/FeedEntryId.java` as a `public record FeedEntryId(UUID recipientId, UUID postId) implements Serializable {}` annotated `@Embeddable`. Provide a no-arg constructor if JPA requires it (verify against the existing `FollowId` pattern from `add-follows`).
...
- [ ] 3.2 Implement `void onPostCreated(Post post)`. SQL:
  ```
...
- [ ] 3.3 Implement `void onPostDeleted(UUID postId)`. SQL: `DELETE FROM feed_entries WHERE post_id = :postId`. No return value.
- [ ] 3.4 Implement `void onFollow(UUID followerId, UUID followeeId)`. SQL:
  ```
  INSERT INTO feed_entries (recipient_id, post_id, author_id, created_at)
...
- [ ] 6.1 Add a package-private `void rebuild()` method to `FeedFanoutService` (annotated `@Transactional(propagation = MANDATORY)`, or split into a separate `@Service FeedRebuilder` — pick based on whether `PROPAGATION_MANDATORY` is too aggressive for the IT helper. Recommendation: a sibling `FeedRebuilder` with its own `@Transactional`.). The method runs:
  ...
  ```
- [ ] 6.2 Note in the class javadoc that `rebuild()` is invariant-test infrastructure, NOT a production code path, and the test that proves the equality (task 7.5) is the only caller.

## 7. Backend: Testcontainers integration tests

- [ ] 7.1 Create `backend/src/test/java/com/prodready/social/feed/FeedControllerIT.java` extending the existing IT base (`PostgresIT` or whatever class `PostsControllerIT` / `FollowsControllerIT` extend). Inject `JdbcTemplate` so cases can read `feed_entries` directly.
- [ ] 7.2 Helper: seed Alice and Bob (signup + login + capture tokens + ids), reuse the existing `useraccounts` test helpers.
- [ ] 7.3 Cases — read path:
...
- [ ] 7.4 Cases — write-path invariants (read `feed_entries` directly via `JdbcTemplate`, no HTTP):
...
- [ ] 7.5 Invariant case: `feedEntries_equalsRebuild_acrossOperations`.
...
- [ ] 7.6 Outside-transaction guardrail case: `feedFanoutService_outsideTransaction_throws` — call any of the four helper methods without an enclosing `@Transactional`. Assert `IllegalTransactionStateException` (or whatever Spring raises for `PROPAGATION_MANDATORY`).

## 8. Backend: extend existing IT suites for the fanout invariants

- [ ] 8.1 Modify `backend/src/test/java/com/prodready/social/posts/PostsControllerIT.java` to add:
...
- [ ] 8.2 Modify `backend/src/test/java/com/prodready/social/follows/FollowsControllerIT.java` to add:
...
- [ ] 9.1 Refresh `openapi/openapi.json` via the repo's existing snapshot-generation task (the same one `add-follows` used — `./gradlew generateOpenApiDocs --no-configuration-cache`).
- [ ] 9.2 Verify the regenerated snapshot includes:
...
- [ ] 9.3 Run Orval against the refreshed snapshot. Confirm:
...
- [ ] 9.4 If the existing CI drift check reports any unexpected diff (e.g. operation re-ordering inside an existing controller), reconcile by re-running the snapshot generator and committing the freshly-deterministic output.
...
- [ ] 10.2 Each rendered post uses `<PostCard post={post} onDeleteSuccess={onFeedItemDeleted} />` where `onFeedItemDeleted = () => queryClient.invalidateQueries({ queryKey: getGetFeedQueryKey() })`.
- [ ] 10.3 Create `frontend/src/features/feed/FeedList.test.tsx` with cases (each `it()` or `test()`):
...
- [ ] 11.1 Modify `frontend/src/features/home/HomePage.tsx` to import `FeedList` from `@/features/feed/FeedList` and replace `<PostList userId={userId} />` with `<FeedList />`. The composer's `authorUserId={userId}` and the welcome card stay.
- [ ] 11.2 Modify `frontend/src/features/home/HomePage.test.tsx` (or whichever file owns HomePage tests; create one if absent):
...
- [ ] 12.1 Modify `frontend/src/features/posts/PostCard.tsx`:
...
- [ ] 12.2 Modify `frontend/src/features/posts/PostCard.test.tsx`:
...
- [ ] 12.3 Modify `frontend/src/features/posts/PostList.tsx` to pass `onDeleteSuccess={() => queryClient.invalidateQueries({ queryKey: postsByAuthorListKeyPrefix(userId) })}` to each rendered `PostCard`.
- [ ] 12.4 Modify `frontend/src/features/posts/PostList.test.tsx`:
...
- [ ] 13.1 Modify `frontend/src/features/profile/ProfilePage.tsx` follow / unfollow mutations:
...
- [ ] 13.2 Modify `frontend/src/features/profile/ProfilePage.test.tsx` (or its follows-side sibling):
...
- [ ] 14.1 Modify `e2e/src/helpers/apiClient.ts`:
...
- [ ] 14.2 Confirm the helper imports from the generated URL-helper module and does NOT hardcode the path string.

## 15. E2E: `e2e/tests/feed.spec.ts`

- [ ] 15.1 Create `e2e/tests/feed.spec.ts` with two top-level `test.describe` blocks: "UI vertical" and "API edges".
- [ ] 15.2 UI vertical `test()` — "Bob's home feed reflects follow / unfollow of Alice":
...
- [ ] 16.1 Modify `e2e/tests/axe.routes.spec.ts` to seed a follow + cross-author posts before the `/home` axe scan. Sign up Alice + Bob; Alice posts 2 posts via `apiClient`; Bob follows Alice via `apiClient.follow(bobToken, aliceId)`; Bob composes 1 post via the SPA; assert (before scan) that `/home` shows 3 `PostCard`s.
- [ ] 16.2 Run `runAxeScan` on `/home` (with the populated feed) and assert no violations. Reuse the existing `runAxeScan` fixture.
- [ ] 16.3 Keep the spec a single `test()` walking the routes sequentially — do not split into multiple `test()`s.

## 17. Spec sync, validate, format

- [ ] 17.1 Confirm `openspec/changes/add-home-feed/specs/feed/spec.md`, `openspec/changes/add-home-feed/specs/posts/spec.md`, `openspec/changes/add-home-feed/specs/follows/spec.md`, `openspec/changes/add-home-feed/specs/user-profile/spec.md` reflect the implementation as shipped. Adjust any drift before opening the PR.
- [ ] 17.2 Run `openspec validate add-home-feed --strict` and resolve any errors.
- [ ] 17.3 Run the backend formatter (`./gradlew :backend:spotlessApply` or whichever task the project uses), the frontend formatter (`pnpm --dir frontend format` / prettier), and the e2e formatter. Confirm the diff is clean.
