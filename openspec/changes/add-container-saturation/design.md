...
There is one platform caveat: cAdvisor in a testcontainer needs `/sys` and the Docker socket. On macOS this works through Docker Desktop's VM (the testcontainer runs *inside* the same VM). On Linux CI runners it works directly. CI does not currently run the observability profile, so this test is dev-only for now; documented in the test's `@DisabledOnOs` or environment guard. Decision deferred to implementation: either gate the test on a system property (`-Dobservability.integration=true`) or always run it and tolerate a longer test suite.
