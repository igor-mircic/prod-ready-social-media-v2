...
- [ ] 7.5 Verify the observability profile still works end-to-end: `just vm-up && just k8s-apply` (postgres-in-k3s) + `docker compose --profile observability up -d`; then hit `http://localhost:9187/metrics` and confirm it contains `pg_stat_database_*` series populated with non-zero values (proving the exporter reached the in-k3s postgres via `host.docker.internal:5432`). <!-- runtime verification — developer-side -->
- [ ] 7.6 Spot-check Prometheus targets at `http://localhost:9090/targets` — the `postgres-exporter` job should be `up`. Spot-check a panel on the slice-12 Database overview dashboard renders real data. <!-- runtime verification — developer-side -->
